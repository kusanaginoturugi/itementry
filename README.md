...
- **商品 (items)**: 商品コード・名前・価格・可変価格フラグ `is_variable_value`・有効フラグ `is_active`・入金報告グループを管理。商品一覧はコード順でソート可能。可変価格の商品だけ単価を明細で編集可。無効化した商品はレシート入力などの候補から除外される。
- **入金報告グループ (report_groups)**: 入金報告書に表示する商品グループ名とコードを登録・編集する。
